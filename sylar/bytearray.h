/**
 * @file bytearray.h
 * @brief 二进制数组(序列化/反序列化),底层使用链表的形式存储数据，每次写入数据时，将数据写入到链表最后一个块中，
 * 如果最后一个块不足以容纳数据，则分配一个新的块并添加到链表结尾，再写入数据。ByteArray会记录当前的操作位置，
 * 每次写入数据时，该操作位置按写入大小往后偏移，如果要读取数据，则必须调用setPosition重新设置当前的操作位置。
 * 
 * @author zq
 */
