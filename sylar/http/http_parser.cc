...
struct _RequestSizeIniter {
    _RequestSizeIniter() {
        // 通过 sylar::ConfigVar 来动态获取和设置这些值
        s_http_request_buffer_size = g_http_request_buffer_size->getValue();
        s_http_request_max_body_size = g_http_request_max_body_size->getValue();
        s_http_response_buffer_size = g_http_response_buffer_size->getValue();
        s_http_response_max_body_size = g_http_response_max_body_size->getValue();

        // 通过使用监听器（Listener）来动态更新 HTTP 请求和响应的缓冲区大小及最大体积限制。
...
HttpRequestParser::HttpRequestParser()
    :m_error(0) {   // 初始化错误状态为 0，表示没有错误

    // 初始化一个新的 HttpRequest 对象，用于存储解析的响应数据
    m_data.reset(new sylar::http::HttpRequest);     

    http_parser_init(&m_parser);                // 初始化 http_parser 结构体
    m_parser.request_method = on_request_method;    // 请求方法的回调函数
    m_parser.request_uri = on_request_uri;          // 请求 URI 的回调函数
    m_parser.fragment = on_request_fragment;        // 请求 fragment 的回调函数
    m_parser.request_path = on_request_path;        // 请求路径的回调函数
    m_parser.query_string = on_request_query;       // 请求查询字符串的回调函数
    m_parser.http_version = on_request_version;     // HTTP 版本的回调函数
    m_parser.header_done = on_request_header_done;  // 请求头完成解析完成的回调函数
    m_parser.http_field = on_request_http_field;    // HTTP 字段的回调函数
    m_parser.data = this;       // 将当前对象指针作为解析器的data传递给回调函数
}
...
size_t HttpRequestParser::execute(char* data, size_t len) {
    // 调用了 http_parser_execute 函数来解析传入的 data 数据，并返回已处理的数据长度（offset），
    // 同时更新原数据指针，确保后续数据能被继续处理。
...
HttpResponseParser::HttpResponseParser()
    ...
    // 初始化一个新的 HttpResponse 对象，用于存储解析的响应数据
    m_data.reset(new sylar::http::HttpResponse);

    httpclient_parser_init(&m_parser);              // 初始化 httpclient_parser 结构体
    m_parser.reason_phrase = on_response_reason;    // 响应原因短语的回调函数
    m_parser.status_code = on_response_status;      // 响应状态码的回调函数
    m_parser.chunk_size = on_response_chunk;        // 响应分块的回调函数
    m_parser.http_version = on_response_version;    // HTTP 版本的回调函数
    m_parser.header_done = on_response_header_done; // 响应头完成的回调函数
    m_parser.last_chunk = on_response_last_chunk;   // 最后一个分块的回调函数
    m_parser.http_field = on_response_http_field;   // HTTP 字段的回调函数
    
    m_parser.data = this;   // 将当前对象指针作为解析器的数据传递给回调函数
}
