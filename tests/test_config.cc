...
void test_yaml() {
    YAML::Node root = YAML::LoadFile("/home/xx/code/C++/sylar-master/bin/conf/log.yml");
    //print_yaml(root, 0);
    //SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << root.Scalar();

    //IsDefined存在返回true,不存在返回false
...
#define XX_PM(g_var, prefix) \
    ...
    YAML::Node root = YAML::LoadFile("/home/xx/code/C++/sylar-master/bin/conf/test.yml");
    sylar::Config::LoadFromYaml(root);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=============" << std::endl;
    sleep(10);
    test_loadconf();    //文件未发生变化，全部跳过了
    //return 0;

    //使用一个 lambda 表达式作为回调函数，用于打印每个配置变量的详细信息。
